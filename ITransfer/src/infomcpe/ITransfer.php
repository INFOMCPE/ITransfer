...
class ITransfer extends PluginBase implements Listener {
    ...
    public function onEnable(){
        ...
        if(!file_exists($this->getDataFolder()."portals.db")){
                 $this->saveResource("portals.db");
   ...
   public function onDisable() {
       ...
       public function onCommand(CommandSender $sender, Command $command, $label, array $args){
                                      ...
                                      if($ip != null){
                                        if($port != null){
                                           $this->transfer($args[1], $sender );
                                             ...
                                             $sender->sendMessage(ITransfer::Prefix.$this->lang('no_port'));
                                        }
                                      }else{
                                          $sender->sendMessage(ITransfer::Prefix.$this->lang('no_ip'));
                                      }
                                break;
                            case 'addsign':
                            $this->session->createSession(strtolower($sender->getName()), 'stat', 3);
                            $sender->sendMessage(ITransfer::Prefix.$this->lang("press_send_1"));
                            $sender->sendMessage($this->session->getSessionData(strtolower($sender->getName()), 'stat'));
                                break;
                             case 'addportal':
                            $this->session->createSession(strtolower($sender->getName()), 'stat', 5);
                            $sender->sendMessage(ITransfer::Prefix."Пожалуйста нажмите на первую точку портала");
                                break;
                            case 'update':
                                $this->signTask();
                                break;
        ...
        public function onChat(PlayerChatEvent $event) {
            ...
            if($this->session->getSessionData($player->getName(), 'stat') == 3){
                ...
                $player->sendMessage(ITransfer::Prefix.$this->lang("press_tap"));
                $event->setCancelled(); 
                }
                if($this->session->getSessionData(strtolower($player->getName()), 'stat') == 7){
                    ...
                    $this->createPortal($this->session->getSessionData(strtolower($player->getName()), 'min'), $this->session->getSessionData(strtolower($player->getName()), 'max'), $message);
                    $player->sendMessage(ITransfer::Prefix.'Успешно. Портал добавлен');
        ...
        public function onMove(\pocketmine\event\server\DataPacketReceiveEvent $event){
            ...
            if ($this->last_x != round($player->getX())){
               ...
               $result = $this->db()->query("SELECT * FROM `ITransferPortals`");
            while ($data = $result->fetch_array(SQLITE3_ASSOC)) {
                if($this->cheakXYZ($player->getX(), $player->getY(), $player->getZ(), $data['min'], $data['max']) == true){
                    ...
                    $player->Teleport( new \pocketmine\math\Vector3($this->last_x, $player->getY(), $player->getZ()));
                }else{
       ...
       public function onPlayerTouch(PlayerInteractEvent $event){
         ...
         if($this->session->getSessionData(strtolower($player->getName()), 'stat') == 1){
             ...
             $player->sendMessage(ITransfer::Prefix.$this->lang("success_sign"));
             } else {
                 $player->sendMessage(ITransfer::Prefix.$this->lang("no_sign"));
             }
         }
             if($event->getBlock()->getId() == 68 || $event->getBlock()->getId() == 63){
	           if($this->session->getSessionData(strtolower($player->getName()), 'stat') != 1){
             ...
             if($this->session->getSessionData(strtolower($player->getName()), 'stat') == 5){
                 ...
                 $player->sendMessage(ITransfer::Prefix.'Успешно. Теперь отмете вторую точку');
                ...
                $player->sendMessage(ITransfer::Prefix.'Успешно. Теперь напишите IP адрес. Порт после ":" Пример: 0.0.0.0:19132');
                $this->session->createSession(strtolower($player->getName()), 'stat', 7);
             }
         
       }
       public function createPortal($min, $max, $ip) {
         $this->db()->query("INSERT INTO `ITransferPortals` (`ip`, `min`, `max`) VALUES ('{$ip}', '{$min}', '{$max}');");
     }
     public function db() {
         return new \SQLite3($this->getDataFolder().'portals.db');
     }
